...
import { apiGetAllProductItems } from "../../apis/product";
...
const NewArrivals = () => {
  ...
  const fetchProducts = async () => {
    ...
    setError(null);
    try {
      const response = await apiGetAllProductItems({ limit: 0 });
      if (!response?.productItemList || !Array.isArray(response.productItemList)) {
        throw new Error("Dữ liệu sản phẩm không hợp lệ.");
      }
      setProducts(response.productItemList);
    } catch (error) {
      setError(error?.msg || "Không thể tải sản phẩm mới. Vui lòng thử lại sau.");
      toast.error(error?.msg || "Không thể tải sản phẩm mới. Vui lòng thử lại sau.");
    } finally {
      setIsLoading(false);
    }
  };
